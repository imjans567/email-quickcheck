...
mode = st.sidebar.radio("Choose Mode", ["CSV", "Manual", "EXE"])
...
def generate_pdf(pred, proba, explanations):
    ...
    for feature, value, importance in explanations:
        content.append(
            Paragraph(
                f"<b>{feature}</b>: {value} (importance {importance:.3f})", styles['Normal']
            )
        )
...
if mode == "CSV":
    ...
    if uploaded_file:
        ...
        df_feat = pd.DataFrame(explanations, columns=["Feature", "Value", "Importance"])
...
# ---------------- EXE MODE ----------------
    ...
    st.subheader("⚙️ Upload Executable File (.exe)")
